...
return new class extends Migration {
    public function up(): void
    {
        Schema::table('service_options', function (Blueprint $table) {
            ...
            if (!Schema::hasColumn('service_options', 'is_default_selected')) {
                $table->boolean('is_default_selected')->default(false)->after('is_multi_select');
            }
            if (!Schema::hasColumn('service_options', 'sort_order')) {
                $table->unsignedInteger('sort_order')->default(0)->after('is_default_selected');
            }
    ...
    {
        Schema::table('service_options', function (Blueprint $table) {
            foreach (['is_active','sort_order','is_default_selected','is_multi_select','is_required','description','group_key','kind'] as $col) {
