...
Route::prefix('v1')->group(function () {
    ...
    // -------------------------------------------------
    // DEV / DEBUG ONLY (⚠️ remove/guard in production)
    // -------------------------------------------------
    ...
    Route::prefix('auth')->group(function () {
        ...
        // NOTE: This is protected so only logged-in users can verify email OTP
    ...
    // Orders (public create + public view) — if you want these to require auth later, move under auth:sanctum.
    ...
    Route::middleware('auth:sanctum')->group(function () {
        ...
        // ---------------------------
        // Vendor application docs (pending vendors allowed)
        // ---------------------------
    ...
    // =================================================
    // CUSTOMER APIs (auth + customer role)
    // =================================================
    Route::middleware(['auth:sanctum', 'customer'])->prefix('customer')->group(function () {

        // Customer order history (supports ?status=&cursor=&per_page=)
    ...
    Route::middleware(['auth:sanctum', 'vendor_or_admin', 'approved_vendor'])->group(function () {
        ...
        // ---------------------------
        // Vendor profile / shop management
        // ---------------------------
        Route::get('/vendors/{vendor}/shops', [VendorShopController::class, 'index'])
        ...
        // Orders (generic status update)
        Route::patch('/orders/{order}/status', [OrderController::class, 'updateStatus']);

        // NOTE: You had this under vendor group — keep if intended.
        Route::get('/services', [AdminServiceController::class, 'index']);

        // ---------------------------
        // Vendor shop scoped routes
        // ---------------------------
        Route::prefix('/vendors/{vendor}/shops/{shop}')
            ->middleware(['vendor_owns_vendor', 'vendor_owns_shop'])
            ->group(function () {
                ...
                /**
                 * Shop Service Options CRUD (children)
                 */
    ...
    Route::middleware(['auth:sanctum', 'admin_only'])->prefix('admin')->group(function () {
        ...
        // App configuration (mobile remote config)
        Route::get('/app-config', [AppConfigController::class, 'show']);

        // App settings CRUD (admin UI)
